...
if [ $use_diffusion -eq 0 ]; then
    python3 ${PACKAGEROOT}/src/fcst.py ${PACKAGEROOT}/net-deterministic/model.keras ${init_time} ${lead_hour} ${member} --no_diffusion --base_dir ${DATAROOT} --output_dir ${DATAROOT}
else
    python3 ${PACKAGEROOT}/src/fcst.py ${PACKAGEROOT}/net-diffusion/model.keras ${year} ${month} ${day} ${hour} ${lead_hour} ${member} --base_dir ${DATAROOT} --output_dir ${DATAROOT}
fi
